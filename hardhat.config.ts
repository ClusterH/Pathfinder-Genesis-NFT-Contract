...
task('generate-root-hash', 'Generates and prints out the root hash for the current whitelist', async () => {
  ...
  const leafNodes = CollectionConfig.whitelistAddresses.map((addr: string) => keccak256(addr))
...
task(
  ...
  async (taskArgs: { newName: string }, hre) => {
    ...
    if (!/^([A-Z][A-Za-z0-9]+)$/.test(taskArgs.newName)) {
      throw new Error('The contract name must be in PascalCase: https://en.wikipedia.org/wiki/Camel_case#Variations_and_synonyms')
    }
